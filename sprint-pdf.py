...
from reportlab.lib.colors import red, yellow, green, blue, orange,violet, black, purple,white, magenta, cyan, darkred, lightblue, darkgreen, darkseagreen, darkblue, darkslateblue
...
def create_story_card(c,story,y_value):
  ...
  c.roundRect(left_edge_x, left_edge_y - y_value, 400, 280, 4, stroke=1, fill=0)
  ...
  try:
  	...
  	try:
	  code =[ x in story[1].lower() for x in ["XXX","interpro","emg ","hmmer","pfam"]].index(True)
	except:
	  code = 5
	
  colors = [None, purple,(0.45,0.62,0.67), darkred, darkslateblue, orange, lightblue]
  ...
  try:
  	...
  	time = "NaN"
  c.drawString(370,430-y_value,"story points: " + str(time))
...
c = canvas.Canvas("sprint-stories.pdf", pagesize=pagesizes.A4)
(width, height) = pagesizes.A4

#generate the story cards

with open('sprint_76.tsv', 'r') as stories:
  linecount = 0
  for story in stories:  
    #print story
    fields = re.split(r'\t+', story.strip())
    if len(fields) < 2:
      continue
    print fields
    if linecount % 2 == 0:
      create_story_card(c, fields,0)
    else:
      create_story_card(c, fields, 350)
      c.showPage()
      print fields
    linecount += 1;
    if linecount > 40:
